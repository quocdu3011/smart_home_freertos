...
constexpr uint32_t BOARD_LINK_STATE_INTERVAL_MS = 1000;
constexpr uint32_t BOARD_LINK_BAUD_RATE = 9600;
...
void sendBoardStateFrame();
void sendBoardRfidFrame(bool granted, const char *uidText);
...
void sendBoardFramef(const char *format, ...) {
  char buffer[128];
  va_list args;
  va_start(args, format);
  vsnprintf(buffer, sizeof(buffer), format, args);
...
void compactUidText(const char *input, char *output, size_t outputSize) {
  ...
  size_t position = 0;
  for (size_t index = 0; input[index] != '\0' && position + 1U < outputSize; ++index) {
...
int16_t toDeciValue(float value) {
  if (isnan(value)) {
    return -1;
  }
...
uint16_t toPositiveDeciValue(float value) {
...
void sendBoardStateFrame() {
  ...
  sendBoardFramef(
    "@STATE,%u,%u,%u,%u,%u,%u,%d,%u,%u,%u",
    snapshot.wifiConnected ? 1U : 0U,
    snapshot.mqttConnected ? 1U : 0U,
    snapshot.wifiPortalActive ? 1U : 0U,
    snapshot.gateOpen ? 1U : 0U,
    snapshot.entryDoorOpen ? 1U : 0U,
    snapshot.climateValid ? 1U : 0U,
    toDeciValue(snapshot.temperatureC),
    toPositiveDeciValue(snapshot.humidityPct),
    static_cast<unsigned int>(snapshot.authorizedCardCount),
    snapshot.lastAccessGranted ? 1U : 0U
  );
}

void sendBoardRfidFrame(bool granted, const char *uidText) {
  char compactUid[UID_TEXT_SIZE];
  compactUidText(uidText, compactUid, sizeof(compactUid));
  sendBoardFramef("@RFID,%u,%s", granted ? 1U : 0U, compactUid);
}
...
void setup() {
  Serial.begin(BOARD_LINK_BAUD_RATE);
  delay(250);
  ...
  for (;;) {
    if (gRfid.PICC_IsNewCardPresent() && gRfid.PICC_ReadCardSerial()) {
      ...
      if (isAuthorizedUid(uidText)) {
        ...
        sendBoardRfidFrame(true, uidText);
        ...
        sendBoardRfidFrame(false, uidText);
...
void taskConnectivity(void *pvParameters) {
  ...
  TickType_t lastBoardStateTick = 0;
  ...
  for (;;) {
    ...
    const bool periodicBoardStateSend =
      static_cast<int32_t>(now - lastBoardStateTick) >= static_cast<int32_t>(pdMS_TO_TICKS(BOARD_LINK_STATE_INTERVAL_MS));

    if (periodicBoardStateSend) {
      sendBoardStateFrame();
      lastBoardStateTick = now;
    }
